...
std::vector<btRigidBody*>& CapsuleHitbox::generateHitBoxes(Model* model)
{
	this->model = model;
	...
	boxCollisionShape = new btCapsuleShape(glm::max(size.x, size.z), size.y);
...
std::array<glm::vec3, 2> CapsuleHitbox::getBiggestHitBox() const
{
...
std::array<glm::vec3, 2> CapsuleHitbox::getMeshCenterAndSize(const std::vector<Vertex>& vertices) const
{
...
void CapsuleHitbox::setRotationAroundCenter(const float angle)
{
...
void CapsuleHitbox::setWorldTransform(const glm::vec3& position, const glm::quat& rot)
{
